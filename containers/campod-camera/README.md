...
Capture container for the Rekon camera pod (Pi Zero 2 W + Camera Module 3 / IMX708). Pulls JPEG stills at a fixed cadence (default 1 Hz) and writes each frame plus a JSON metadata sidecar to the Zero's **local SD card** (never over USB -- the USB 2.0 bus is for commands only; see [`campod.md`](../../docs/campod.md)).
...
| Base image | `debian:trixie-slim` + the **Raspberry Pi apt archive** (`archive.raspberrypi.com`) for matched, Pi-pipeline-aware libcamera + `python3-picamera2`. Stock Debian libcamera enumerates "no cameras" -- the one real container gotcha. **`RPI_SUITE` must match the suite the campod image is built from** (`dotfiles-symm/pi-image/build-image.sh`): the container's libcamera talks to the host kernel, so the two move together or not at all. |
...
| Build | arm64 in CI ([`.github/workflows/build-containers.yaml`](../../.github/workflows/build-containers.yaml)), pulled on the Zero -- never built on the Zero. |

## Config (in `stacks/campod/compose.yaml`)

| Var | Default | Meaning |
|-----|---------|---------|
| `CAMPOD_NODE_NAME` | the **host's** name, from `/etc/host-hostname` | label in filenames + metadata. Deliberately **not** the container's hostname, which is the container ID and changes on every recreate; the stack file bind-mounts the host's `/etc/hostname` so both readers can see it. Set nowhere -- a literal here is a per-unit value in a file describing four pods, which is what #272 was. |
