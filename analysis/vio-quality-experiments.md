...
- **E35** measures the front end this fix would consume — stereo VO from the tracker's features, VINS
  out of the loop, on 260814. On **position** it is **worse** than the published pose (aligned ATE
  0.35 m vs 0.17 m, scale 0.889 vs 0.910), which agrees with E26. On **attitude** it is better in the
  median (8.1° vs 12.9°) but not consistently, and by nothing like E25's margin. So this measures the
  front end without settling what it is worth; the operator's stated direction is to hand the FC
  **deltas with an honest covariance** rather than a pose, and let the EKF reject high-innovation
  spikes as it already does for GPS — which is a different interface from the one E35 scores.
- **Probes:**
...
- **E34** measures band pitch by a scene-independent route. Within 260814 it is tight (**895 rows**,
  IQR 880-901); on 260730, the only other flight sharing the same mount, it is internally unstable --
  so the method is not yet reliable flight-to-flight and settles nothing about band=rev. It does raise
  a candidate this theory does not consider: the bands may be an **isolator mount resonance** rather
  than a motor order. Testing pitch against RPM does **not** need a new flight -- throttle varies
  within every flight; it needs the method to register pairs outside a hover.
- **Probes:**
...
| **E30** | *[method]* **Putting capture data on the FC clock, and the NTP step that breaks the naive join.** The coordinator wall clock **steps once** when NTP first disciplines it after boot: on 260814, **+184.282 s at monotonic 69.54 s**, which the coordinator journal independently dates to monotonic 69.52 s (`systemd-timesyncd`, *"Initial clock synchronization"*, after three NTP timeouts against the public pool). Sidecars and `.feat` datagrams recorded before that step carry wall-clock stamps 184 s early, which reads as a 185 s hole in the capture cadence -- there is no hole, `monotonic_ns` and `seq` are both continuous. **Join on monotonic, and repair the pre-step wall stamps from it.** The bridge to FC time needs no GPS arithmetic: the FC logs `VISP.RTimeUS`, the coordinator's own wall-clock stamp for each pose it received, alongside its own `TimeUS`, so regressing one on the other gives coordinator-wall -> FC-time directly. On 260814 that fit has a residual of **1.2 ms rms, 3.8 ms max** over 2652 samples. | `analysis/capture_align.py` via `analysis/estimator-input-audit.ipynb` on the 260814 `.bin` and `.feat`; journal cross-check from `journal-260905.tar.gz` | **Method, not a theory.** Speaks to [#167](https://github.com/symmatree/coordinator/issues/167), which records the capture-timestamp disagreement as "~5 s, untested"; complements E13 (clock *drift*, where this is *offset*). The 1.2 ms is the **fit residual**, not an accuracy claim against absolute UTC. The step size is a per-boot NTP event and will differ on other flights; the module detects it rather than assuming this value. |
| **E31** | *[method + result across the corpus]* **Every `.feat` fixture recorded with the capture overlay running is missing 38-59% of the frames the camera produced.** Feature-frame **device** timestamps sit exactly on the 20 fps grid (grid residual median **0.0000 s** across all 11 fixtures on the NAS), so implied-vs-emitted frames are countable: 260705-handheld **0.0%**, 260705-vio-logged **0.0%**, 260812-hover/113912 **2.0%**, 260812-hover/114548 **1.5%** -- all four wrote no capture artifacts -- against 260812-hover/113948 **38.1%**, 260724-bench **43.8%**, 260712-crash **44.5%**, 260814-woods **48.9%**, 260812-maneuver **49.1%**, 260728-sunny **55.7%**, 260730-rubber **58.8%**, all of which did. No overlap between the groups; among the six flight sessions that wrote images, loss against images/second gives **r = 0.85**. On 260814 the loss is visible end-to-end: `VISP` carries exactly the same **2445** samples and the same 42 (>1 s) / 20 (>2 s) gap counts as the `.feat`, and all **101** `VISP` gaps >0.5 s coincide with a `.feat` gap within 0.5 s (median offset 0.05 s). | `analysis/feat_stream.py` (`frame_loss`) over every `.feat` on the NAS -- **whole-fixture**, not airborne-gated, so the figures are comparable across sessions of different lengths; the airborne-gated figure for 260814 is **54.9%** (`analysis/estimator-input-audit.ipynb`). Plus 260814 FC `.bin` (`VISP`) and `journal-260905.tar.gz`; detail in [#156](https://github.com/symmatree/coordinator/issues/156) | **Methodology confounds** (see the new entry there) and [#156](https://github.com/symmatree/coordinator/issues/156). **No cause named.** The separation is observational, not a controlled A/B: 260705 (0.0%) predates the capture overlay *and* differs in onboard software and ArduPilot build (the same confound E23 flags) *and* its fixture came from the external `vio-ipc-record` rather than the in-tracker tee. The two 0-artifact 260812 sessions are disarmed and stationary. The nearest thing to a control is **inside 260812-hover** -- same day, same build, same hardware, three consecutive sessions at 2.0% / 38.1% / 1.5%. Arm-gating ([#88](https://github.com/symmatree/coordinator/issues/88), 2026-07-30) makes "wrote images" and "was armed" perfectly correlated in every later session, so capture load and motors/vibration are separated only by 260705-vio-logged (armed, motors on, hard-mounted, 0.0%), which carries the build confound above. What is *excluded* on 260814: power/thermal (`coord-throttle: 0x0 [clean]` at all ten journal samples through the flight) and kernel-visible USB faults (zero kernel messages after monotonic 30 s). **Which rows sit on which fixture:** E10/E11/E23 rest on the two **260705** fixtures (**0.0%** loss), E22 on a **260730** fixture (**58.8%**), E16/E17/E19 on **260712** (**44.5%**). Recorded as provenance, not as a challenge to any of them -- offline replays the same file, so online and offline see the same gaps, and E16's 6 mm agreement is unaffected. This is a data-collection bug ([#156](https://github.com/symmatree/coordinator/issues/156)), not a replay-fidelity question. |
| **E32** | *[flight datapoint, 260814]* **Under-canopy feature supply and scene depth, from the `.feat` (260814).** Airborne-gated (t 75.3-345.9 s, derived physically by `capture_align.airborne_window`). Stereo-matched features/frame: **median 26**, p05 14, min 5, against `setNumTargetFeatures(16*5)=80`; by regime **open lawn 34, entering woods 24, deep woods 18, exit 29**, with only **0.8%** of deep-woods frames below 10 features. Sub-pixel disparity over 58,268 airborne feature observations: p25 1.30 / **median 2.39** / p75 4.56 px, **41.6% under 2 px**; depth **median 12.6 m**, **58.3% beyond 10 m**; by regime median depth 19.0 m open, 11.3 m entering, **10.2 m deep in**, 7.5 m on exit. Independent cross-checks from the same stream: `fx` recovers to **400.49 px**, `cx` **311.47** (E29: 400.5 / 311.5), and rectified stereo rows agree to **0.39 px** median. | `analysis/estimator-input-audit.ipynb` (`feat_stream.depth_stats` / `feature_counts`, airborne window from `capture_align`) | **T12 / E27, T6, E19, E28.** Compare E27 (260812-maneuver, open field): 2.25 px median, 45% under 2 px, 14.7 m median, 64.5% beyond 10 m. Under canopy is **only marginally closer-in** than the open-field flight T12 was raised on. Feature supply shows a clear ~2x scene effect but **no E19-style collapse** (E19: ~10x collapse to ~4 features). Ground frames were excluded, and they are a different scene -- on-ground disparity median 20 px (Z ~1.5 m) against 2 px airborne, on-ground feature median 14 against 26 -- but on **this** flight they are only 3% of the capture set (arm-gating, [#88](https://github.com/symmatree/coordinator/issues/88)), and excluding them leaves the airborne medians unchanged to 2 decimal places. The gating matters for the pre-gating flights, not here. |
| **E33** | *[flight datapoint, 260814]* **The rectified-mono VIO input, saved for the first time, under canopy (260814).** 248 `mono_rect_left` frames ([#125](https://github.com/symmatree/coordinator/issues/125); no other capture session on the NAS has them). Mono auto-exposure ran **580 us in the open to a hard 10,000 us ceiling under canopy at ISO 100-112** -- minimum gain throughout, i.e. AE spent shutter and never traded to gain. `OAK_MONO_MAX_EXPOSURE_US` is `0` (uncapped) and `monoLeft->setFps(20)` would permit 50 ms, so **what pins it at exactly 10 ms is unexplained**. On inspection at 1:1 the frames are sharp with abundant texture, including the 9992 us deep-canopy frames; a var-Laplacian score over the 239 airborne frames correlates **+0.25** with exposure (i.e. weakly *positive*, not the negative a blur mechanism would give) and shows no relationship to `VIBE`. Separately, the colour-still exposure cap is live and working: **all 45 stills at exactly 4996 us** with ISO carrying the range 162 to a pinned 1600 -- the first hardware test of X15 / PR #105. | 260814 `mono_rect_left` PNGs + sidecars; still sidecars; `image-sharpness-vs-motion` scorer | **E28, T9, T6, [#158](https://github.com/symmatree/coordinator/issues/158).** E28 recorded that no flight with captures had ever seen low light on mono, so the dark case was *unmeasured, not excluded*, and predicted that an uncapped mono AE running long under canopy would blur the VIO input. The AE did run long (10 ms, 24x the open-field p50) and, on this flight, **the frames do not look blurred** -- but note the cameras are **global shutter and fixed focus** ([`docs/rekon10/oak-d-mount.md`](../docs/rekon10/oak-d-mount.md)), and at 640x400 the angular sampling is 6.3x coarser than the 12 MP still, so the mono is structurally far less exposed to both jello and the still's blur mechanism. **No claim is made here about whether 10 ms would blur under harder motion**; this flight's speeds were <=1.7 m/s. |
| **E34** | *[method; not yet reliable across flights]* **Scene-cancelled still-banding measurement.** Two stills of the same scene are registered by phase correlation; the **ratio** of per-row gradient energy between the registered frames divides the scene out; a dense sinusoid fit over that ratio gives a band pitch. On **260814** (stills seq 4-8, during a stationary hover -- 0.02 m/s, 0.2 deg/s body rate, same 5 ms exposure and ISO 331) the ratio is a clean x0.2-x4 oscillation with the two frames' per-row sharpness near **anti-phase**, and the fit gives **895 rows, IQR 880-901** over the 4 pairs passing the quality filter. On **260730** -- same isolators, same mount -- it gives **1841 rows but is internally unstable** (IQR 1210-1856, n=10). On **260728** it gives 1935 rows (IQR 1817-1981, n=7), but that flight **predates the rubber isolators** so it is a different mechanical path and not comparable. | `analysis/still_banding.py` (`pose_pairs` + `session_pitch`), per-flight via `analysis/estimator-input-audit.ipynb`; candidate pairs selected by **pose** (within 1.0 m and 10 deg, >=2 s apart) rather than adjacency in time, then kept at phase-correlation peak >0.10 and sinusoid R^2 >0.60; 260812-maneuver produced no pair above threshold | **T11 / E24, T10, T3.** The only same-mount comparison available is **260730 vs 260814**, and there one measurement is tight and the other is not stable within itself -- so the honest read is that **the method is not yet reliable flight-to-flight**, not that the physics differs. Nothing about band=rev is settled either way by this. What survives: within one flight at one mount state the pitch is tight, and the structure is **camera-side rather than scene** (the ratio cancels the scene); E24's over-counting caveat also reproduces (single-frame detectors on 260814 spread over 62-281 Hz and fire on ground frames). **Candidate this raises:** an isolated camera on a rigid airframe is a mass-spring system with a natural frequency set by isolator stiffness and preload, so the bands may be a **mount resonance** rather than a motor order. **The discriminator is already in the data we have** -- throttle varies within every flight (there is no slow sweep available, but there are climbs and vertical leaps), so band pitch against per-frame RPM is testable *within* one flight once the method can register pairs outside a hover. That registration limit, not a missing flight, is what currently blocks it. **Nothing here should be load-bearing for a hardware or flight-profile decision.** **Side observation worth its own follow-up:** selecting candidates by pose surfaces that **13 of 19 pose-close still pairs on 260814 do not register at all**, including pairs **4.8 cm apart with 0.01 deg of yaw difference** (phase-correlation peak 0.09, under the 0.10 floor). The geometry says those frames see the same scene; they do not match. Candidates are heavy blur leaving little common high-frequency content, band phase differing between the two so the sharp rows do not overlap, exposure/ISO drift (290-331 across that hover), or simply **wind moving the foliage** over the 15-39 s separations involved -- the last being the obvious one for a scene that is entirely trees. A hover facing a **brick wall** removes the scene-motion explanation and would discriminate. |
| **E35** | *[method + first result]* **Step A -- frame-to-frame relative pose direct from the tracker features, VINS out of the loop (260814).** Triangulate each stereo-matched point from the undistorted normalised coords, match to the previous frame by tracker feature id, solve rigid pose with RANSAC (inlier threshold scaled with scene depth) and, separately, rotation alone from unit **bearing** vectors. **2374/2438 pairs solve (97%)**, median 22 shared points, median 14 inliers, 3D-3D residual **0.186 m** at 12.9 m median depth, bearing-rotation residual **0.056 deg** per pair. Integrated, the airborne window breaks into **17 segments** (E31), median duration **8.8 s**, and comparison to the FC per segment with `VISP` scored over the identical windows gives, over the **10 segments with >2 m true extent**: aligned ATE **0.35 m local vs 0.17 m `VISP`**, Umeyama scale **0.889 vs 0.910**, integrated attitude drift **8.1 deg local vs 12.9 deg `VISP`**. Over the hover segment (92.3-122.3 s, 391 poses, EKF extent 0.19 m) where the vehicle drifted **0.0029 m/s**: local **0.0563 m/s**, `VISP` **0.0877 m/s**. | `analysis/local_pose.py` via `analysis/estimator-input-audit.ipynb` on the 260814 `.feat` and `.bin`; deterministic (fixed RANSAC seed) | **T8, T12, E25, E26, X23.** **On position this agrees with E26: the local solve is not better** -- it is worse than the published pose, and both scale near 0.9. On attitude it is directionally consistent with E25 but **far weaker**: a median 8.1 vs 12.9 deg, with `VISP` actually better on several segments, against E25's 6.75 vs 142-158 deg. **Caveats, and they are load-bearing here:** segments are short (median 8.8 s, median extent ~4 m), so Umeyama is weakly constrained and the E26 degeneracy lesson applies directly -- the longest segment by duration (29.9 s) has only **0.19 m** of true extent and its fitted scale of 0.015 is meaningless. The camera-to-body rotation is taken as an exact axis permutation and is **not calibrated**; the airframe also parks tilted (battery-strap buckle), so a ground segment cannot be used to calibrate it either. All numbers are post-alignment, under the doc's standing caveat. **An earlier draft of this row quoted 3.59 deg over a 47.4 s segment against 51.30 deg, and fit-scales of 0.708/0.860; those came from a one-off script whose segmentation the checked-in module does not reproduce, and they are withdrawn.** |
