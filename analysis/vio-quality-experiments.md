...
>
> **UPDATE 2026-09-06 — the 260814-woods captures came off the aircraft.** `260814-woods/` was
> previously FC-log-only (E28 noted its coordinator-side captures had never been pulled). They were
> offloaded 2026-09-05 and are on the NAS: 45 colour stills, **248 `mono_rect_left` frames**, 251
> disparity frames, the `.feat`, and the coordinator's `/var/log/journal`. Three things make this the
> most complete single run to date, independent of what it shows: it is the **first flight with the
> rectified-mono VIO input saved** ([#125](https://github.com/symmatree/coordinator/issues/125) —
> no other session on the NAS has it, and E18/E28 both record its absence as a limit); it is the
> **first flight under real canopy** carrying `.feat` + captures + `VISP` + journal together; and it
> flew the [#138](https://github.com/symmatree/coordinator/issues/138) parameter package (E36).
> Derived figures and a full method write-up live in that flight's `derived/README.md` on the NAS.
> Observations recorded as **E30–E36**; the theory statuses above are unchanged pending a separate pass.
>
...
- **New data point (260814, 2026-09-06 — recorded, status not reassessed):** under real canopy the feature
  count shows a clear scene effect but **no collapse** — airborne median 26/frame, deep-woods median 18,
  only 0.8% of deep-woods frames below 10 (**E32**). Separately, the tracker's feature *output* was absent
  ~52% of the airborne window, and that loss is upstream of the scene
  (**E31**, [#156](https://github.com/symmatree/coordinator/issues/156)) — so
  "feature health" now has two distinguishable axes, **count** (this theory's metric) and **continuity**,
  which prior rows did not separate.
- **Probes:**
...
- **New data point (260814, 2026-09-06 — recorded, status not reassessed):** **E35** reproduces the E25/E26 axis split
  on a second flight, under canopy, at 3x the solve rate: rotation from the raw feature stream is
  excellent (3.59° over 47 s against `VISP`'s 51.30° on the identical window) while position from the
  same stream is **worse** than the published pose (ATE 0.49 m against 0.16 m) and under-scales
  (fit-scale 0.708 against 0.860). Recorded here because it bears on *which* signal an alternative
  architecture would be taking from the front end; no change to the theory's status is claimed.
- **Probes:**
...
- **New data point (260814, 2026-09-06 — recorded, status not reassessed):** **E32** puts the equivalent geometry
  numbers on a real under-canopy flight, from the `.feat` sub-pixel stream: disparity median 2.39 px
  (41.4% under 2 px), depth median 12.6 m (58.2% beyond 10 m), deep-woods median 10.2 m — against
  E27's open-field 2.25 px / 45% / 14.7 m / 64.5%. **Marginally closer-in, not qualitatively different.**
  This is the measurement E28 flagged as retrievable once the 260814 captures came off the aircraft.
  **E35** adds that the shared under-scaling reproduces with 3x the feature solve rate, so it does not
  look feature-supply-limited.
- **Probes:**
...
- **New data point (260814, 2026-09-06 — recorded, status not reassessed):** first hardware data on the exposure
  lever (**E33**): X15's cap is live and **all 45 stills came in at exactly 4996 us**, with ISO carrying
  the range 162 → a pinned 1600. The banding survives that cap (**E34**), which is expected rather than
  surprising — at ~102 Hz a half-cycle is 4.9 ms, so a 5 ms exposure still integrates close to a full
  peak-to-peak excursion. Also relevant to the *region* this theory is defined over: airframe speeds on
  260814 were ≤1.7 m/s and body rates mostly <1 deg/s, so translation-driven blur is small there and the
  flight is a weak test of the speed axis. Mono, by contrast, ran to a 10 ms ceiling and does **not** look
  blurred (**E33**) — but it is global-shutter, fixed-focus and 6.3x coarser, so it is not a control for
  the still.
- **Probes:**
...
- **New data point (260814, 2026-09-06 — recorded, status not reassessed):** **E34** cannot exclude focus modulation
  *during readout*. What it does exclude is a **static or whole-frame** focus error: the blur alternates
  in 901-row bands within one frame and shifts phase between two frames 10 s apart, which a per-frame
  focus state cannot do. A structure-tensor test for isotropic (defocus-like) versus directional
  (smear-like) blur inside the bands was **inconclusive** — coherence is lower in the blurred bands,
  which reads defocus-like, but a band with ~4x less gradient energy sits nearer the isotropic noise
  floor whatever the cause. `containers/vio-tracker/README.md` already records the Luxonis note that
  under high vibration the AF coil cannot hold the lens even in manual focus. **`OAK_STILL_FOCUS=125`
  (PR #205, merged 2026-09-05, after this flight) makes the next flight a free comparison** of commanded
  VCM motion against unintended lens movement.
- **Probes:**
...
- **New data point (260814, 2026-09-06 — recorded, status not reassessed):** **E34** measures the band pitch by a
  scene-independent method — the per-row sharpness **ratio between two registered frames of the same
  scene**, which divides the scene out — giving **901 rows, sd 57 over 8 pairs**, i.e. 102.7 Hz ± 6.4 at
  the assumed 33 ms readout. That is a much tighter measurement than E24's visual 4-band count and it
  **excludes blade-pass**. It does **not** identify the source: all 8 pairs sit in one ~40 s hover at
  essentially one RPM, so band-pitch-versus-RPM — the actual discriminator — remains untested, and the
  readout time is still assumed rather than measured. E34 also independently **reproduces this theory's
  over-counting caveat**: single-frame detectors on 260814 spread over 62–281 Hz and fire on ground frames.
- **Probes:**
...
- **Open (new 2026-09-06) — the fixtures themselves are missing 38–59% of their frames (E31).** Every
  `.feat` recorded with the capture overlay running lost 38.1–58.8% of the frames the camera produced;
  the four sessions that wrote no capture artifacts lost 0.0–2.0%. This is a property of the *input* that
  every offline result was computed on, and it was not known when those rows were written. It does **not**
  invalidate them — offline replays the same file, so online and offline see the same holes, and E16's
  6 mm online/offline agreement is untouched — but it narrows their scope: a claim of the form "the
  estimator diverges" is demonstrated as "the estimator diverges **given this input**." Two rows are
  affected unequally and it is worth knowing which: **E10/E11 rest on the two 260705 fixtures (0.0% loss)**,
  while **E22 (260730) rests on a fixture at 58.8%** and **E23 (260705) at 0.0%**. No conclusion is drawn
  here about whether a complete stream would change any of those results; that is untested. Cause is
  unattributed — see E31's caveats and [#156](https://github.com/symmatree/coordinator/issues/156).
- **Open (new 2026-09-06) — "in-flight" has meant "armed", which is not the same thing.** Capture
  arm-gating landed 2026-07-30 ([#88](https://github.com/symmatree/coordinator/issues/88)); sessions before
  it recorded from coordinator boot. On 260814, ground frames differ materially from airborne ones —
  disparity median 20 px (Z ~1.5 m) against 2 px airborne, feature count median 14 against 26 — so mixing
  them moves both scene-depth and feature-supply numbers, in opposite directions from what one might guess.
  260814 is only lightly affected (9% of stills, 4% of mono/disparity) because of arm-gating, and E32/E33
  are airborne-gated. **The pre-2026-07-30 flights (260705, 260709, 260712, 260728) have not been checked
  for this**, which touches E18's "0/29 in-flight frames" and E28's in-flight exposure percentiles. Also
  note the gate itself needs care: `EV Id=28` fires 3.7 s *before* liftoff (it marks spool-up), and an
  altitude threshold mis-detects touchdown when the landing spot sits below the home datum, as it did here.
