...
- **Open (new 2026-09-06) — capture sets recorded before arm-gating are a quarter to a half
  ...
  260728/260730/260812 are the concrete instance to re-check; **E18 is not affected**, since it
  deliberately contrasts at-rest against in-flight. (No figure is quoted for 260728: its liftoff
  detection was inconsistent in this pass — detected lift precedes the ARM record — and was not chased
  down.) Method note for anyone deriving the window: `EV Id=28` fires before liftoff (it marks
  spool-up), and an altitude threshold mis-detects touchdown when the landing spot sits below the home
  datum, as it did on 260814.
