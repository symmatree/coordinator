...
- **Status:** **vibration disfavored as the cause (E22, 2026-07-30).** The rubber-isolated flight
  (260730) is the direct isolation discriminator the doc wanted: isolation **halved** the OAK-D >5 Hz
  accel (226 vs 444 band-RMS), yet `imu:1` **still ran away identically** (20,149 m) -- a 2× vibration
  reduction bought *zero* improvement, so it is not a dose-response. (Not a *total*-isolation test -- E1
  puts armed at ~400-500× handheld, so 2× is partial -- but the failure is instant, not threshold-nudged.)
  Combined with E2 (handheld also fails): **vibration is not shown to be the cause.** The remaining
  candidate is the IMU data-path/model (T4/T5), and E22 narrows *that* toward **T5** (below).
...
- **Status:** **the IMU-fusion path is where it breaks (strongly supported); now narrowed toward T5
  (IMU model/data), away from T4 (extrinsic), by E22.** On 260730, `imu:1` with a **fixed** extrinsic
  (`estimate_extrinsic:0`) and with **online calibration** (`:2`) gave **bit-identical** 20,149 m
  runaways -- the extrinsic estimation is not the driver; the divergence (`numerically unstable in
  preintegration`) happens before the extrinsic could engage. Points at the **IMU feed/model** (T5;
  cf. E4's BNO085 fused-output signature) over the extrinsic (T4). *Not fully isolated* (time-sync and
  the fused-IMU noise model are still entangled), but T4-as-primary is weakened. Discriminator to close
  it: X7 (raw high-rate IMU) vs the current fused BNO085 product.
...
| **E22** | **Rubber isolation does NOT rescue `imu:1` -- it's the IMU feed, not vibration or extrinsic (260730).** Same isolated-flight `.feat` (`20260730T115037Z`), three offline `vins_fusion_offline` runs: **imu:0** stereo-only **bounded 69 m** (flight data is fine); **imu:1** (`estimate_extrinsic:2`, the pre-#69 default) **runs away to 20,149 m / 239 m/s**; **imu:1** with a **fixed** extrinsic (`estimate_extrinsic:0`) gives the **bit-identical** 20,149 m runaway. The isolation **was effective** -- it halved the OAK-D >5 Hz accel band-RMS (**226** vs **444** on non-isolated 260705). Failure mode: **`numerically unstable in preintegration`** from the first frames. | native amd64 rootfs (`pull_estimator_rootfs.py`, Option B1) + `.feat` IMU/feature streams | **−T3** (2× less vibration -> zero improvement; not a dose-response), **−T4** (fixed == calibrated extrinsic -> extrinsic estimation is not the driver), **+T5** by elimination (the IMU preintegration itself diverges, independent of vibration + extrinsic -- consistent with E4's BNO085 fused-output signature). CAVEAT: one flight; 2× isn't total isolation (E1: armed ~400-500× handheld); the fixed==calibrated identity is *because* divergence precedes extrinsic engagement |
