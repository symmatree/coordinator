...
- **E34** measures band pitch by a scene-independent route, and applying it to the older flights does
  **not** support the identification. Within 260814 it is tight (**895 rows**, IQR 880-901). Across
  flights at essentially the same hover rev line (**114.5 / 116.9 / 116.0 Hz**) it is not: **1935 rows**
  on 260728-sunny and an internally unstable **1841** on 260730-rubber. A rev line written through a
  fixed readout predicts constant pitch when the rev line is constant. Whether the bands are not the
  rev line, or the method is not yet measuring one thing, is open.
- **Probes:**
...
| **E34** | *[method; does not yet reproduce across flights]* **Scene-cancelled still-banding measurement.** Two stills of the same scene are registered by phase correlation; the **ratio** of per-row gradient energy between the registered frames divides the scene out; a dense sinusoid fit over that ratio gives a band pitch. On **260814** (stills seq 4-8, during a stationary hover -- 0.02 m/s, 0.2 deg/s body rate, same 5 ms exposure and ISO 331) the ratio is a clean x0.2-x4 oscillation with the two frames' per-row sharpness near **anti-phase**, and the fit gives **895 rows, IQR 880-901** over the 4 pairs passing the quality filter. **Applied to the older flights the number does not hold:** **260728-sunny 1935 rows** (IQR 1817-1981, n=7) and **260730-rubber 1841 rows but internally unstable** (IQR 1210-1856, n=10) -- against hover rev lines of **114.5 / 116.9 / 116.0 Hz**, within 2% of each other on all three. 3-blade blade-pass would be ~300-360 Hz on all three. | stills + `ESC` decode on 260814 / 260728 / 260730; pairs kept at phase-correlation peak >0.10 and sinusoid R^2 >0.60; 260812-maneuver produced no pair above threshold | **T11 / E24, T10.** **This does not support the rev-line identification.** A motor rev line written through a fixed readout predicts a pitch that is constant when the rev line is constant; it is not -- two flights give tight but different values a factor ~2.2 apart, and a third is unstable within itself. Either the bands are not the 1st-order rev line, or the method is not yet measuring one thing. **Both are open and no cause is named.** The older flights also differ in more than date: 260730 is the rubber-isolation trial, so the mechanical path to the sensor was deliberately changed. What *is* reproduced is the qualitative structure E24 describes and the fact that it is camera-side rather than scene (the ratio cancels the scene), plus E24's over-counting caveat -- single-frame detectors on 260814 spread over 62-281 Hz and fire on ground frames. **Until the pitch reproduces, nothing here should be load-bearing for a hardware or flight-profile decision.** |
