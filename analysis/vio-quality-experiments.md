...
| **E19** | **Feature starvation coincides with the 260712 divergence.** The tracker's `.feat` **feature payload collapses ~10×** (peak ~4600 B ≈ ~44 feats at motion onset → floor ~330–430 B ≈ ~4 feats) around the divergence (feat-mono ~280–310 s ≈ FC ~452 s, within gyro-sync tolerance). The slide is **bounded** — VISP ≤ ~52 m, plateaus ~40–47 m (**not** the E15 megameter) — and `PErr` stays pinned at **0.2** throughout (fail-confident, cf. E15). Ground truth: GPS **RTK-float throughout** (not a driver), altitude **~1–2 m** at divergence (the ~10 m peak is the terminal strike at ~556 s), flight GPS-primary so VIO never drove. Scene at divergence (color still + disparity PNGs): **dense backlit forest** (trunks + bright sky-gaps + foliage), **not open sky** | `.feat` payload-length series + FC `.bin` (`VISP`/`GPS`/`XKF1`) + capture stills/disparity (260712) | **[evidence]** feature count collapses ~10× coincident with the divergence; the failure is a bounded slide, fail-confident. **[conjecture]** feature loss *causes* the divergence (coincident + plausible mechanism, causation unproven, ±clock-sync); scene-difficulty (high-DR forest) *causes* the feature loss (the **mono VINS input is not saved**, E18 — exposure/occlusion/texture/blur unresolved). **[flag]** this flight's derived `oak_d.yaml` is **`imu:0` (stereo-only)** — reconcile with E17's `imu:1` divergence attribution |
