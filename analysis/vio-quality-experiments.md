...
- **Status:** **vibration-as-rescue unsupported at 2× isolation; not disproven (E22/E23, 2026-07-30).**
  ...
  **near-zero-vibration** test (X14) is still unrun. **Not shown to be the cause, not ruled out.** The live
  candidate is the IMU data-path/model (T4/T5, below).
...
- **Status:** **the IMU-fusion path is where it breaks (strongly supported); the extrinsic sub-driver
  (T4) is untested, not excluded (E22/E23).** On 260730, `imu:1` with a **fixed** extrinsic
  (`estimate_extrinsic:0`) and with **online calibration** (`:2`) gave **bit-identical** 20,149 m runaways.
  The bit-identity means the divergence (`numerically unstable in preintegration`) fires **before the
  extrinsic estimator ever engages** -- so this run **cannot test T4 either way** (an unengaged estimator
  can be neither credited nor blamed); it does *not* show "extrinsic is not the driver." What the runs *do*
  show: the break is in the **IMU-fusion path** and is **input-robust** -- E23 gets the same runaway on the
  continuous, gap-free 260705 stream **with no instability warning**, so not gaps and not vibration. That
  is consistent with **T5** (IMU feed/model; cf. E4 fused-gyro), but T4 stays open. Discriminators to
  actually separate them: X7 (raw vs fused IMU) and a run where the extrinsic **demonstrably engages**
  before divergence.
...
| **E22** | **Rubber isolation does not rescue `imu:1` (260730).** Same isolated-flight `.feat` (`20260730T115037Z`), offline `vins_fusion_offline`: **imu:0** stereo-only **bounded 69 m**; **imu:1** (`estimate_extrinsic:2`, pre-#69 default) **runs away 20,149 m / 239 m/s**; **imu:1** with a **fixed** extrinsic (`estimate_extrinsic:0`) is **bit-identical** (20,149 m). Isolation was mechanically real -- it **halved** the OAK-D >5 Hz accel band-RMS (**226** vs **444**, non-isolated 260705). Logged failure mode: **`numerically unstable in preintegration`** from the first frames. | native amd64 rootfs (`pull_estimator_rootfs.py`, Option B1) + `.feat` IMU/feature streams | Isolation at **this dose does not rescue** `imu:1` -- but 2× is far from zero (E1: armed ~400-500× handheld), so this is one partial-dose point that **weakens vibration-as-rescue without disproving T3**; near-zero-vibration (X14) is the real test. The fixed==calibrated **bit-identity means the extrinsic estimator never engaged** (divergence precedes it), so this run **cannot test T4** -- neither supports nor refutes it. Net: **consistent with T5** (IMU-preintegration divergence, cf. E4), **not a positive proof**. Open discriminators: X14 (near-zero vibration), X7 (raw vs fused IMU). |
| **E23** | **`imu:1` runs away on gap-free input too (260705 clean control).** The pre-in-tracker-capture 260705 `.feat` carries a **continuous ~100 Hz** IMU stream (no stalls -- vs the ~5 s-periodic pipeline stalls in every post-capture flight, 260712/28/30). `imu:1` on it **runs away 25,910 m / 686 m/s** with **zero** `numerically unstable` warnings -- it diverges **smoothly** (bounded ~30 s, then integrates away), a *different* signature from the gappy flights' preintegration instability. | native amd64 rootfs (`pull_estimator_rootfs.py`) + 260705 `.feat` | **Excludes capture-stalls/gaps** as the cause of the `imu:1` runaway -- it fails on clean, continuous, warning-free input. **Strengthens T5**: the divergence is inherent to the IMU fusion/init/scale (cf. E3 over-scale-from-takeoff), robust to gaps *and* vibration. The gaps are a real but **separate** problem (safety, [#156](https://github.com/symmatree/coordinator/issues/156)), not this one. |
