...
| `capture: WARNING could not pin exposure` | container libcamera predates the exposure/gain mode split (needs >= 0.4). Check `RPI_SUITE` matches **the suite the campod image is built from** (`dotfiles-symm/pi-image/build-image.sh`) |
| libcamera reports "no cameras" | **First check the ribbon** -- both ends, contacts toward the board. If libcamera prints its version banner at all, the suite pairing is fine and the camera is simply not attached. A genuine suite mismatch shows as libcamera failing to initialise, not as an empty camera list. |
