...
**Photogrammetry baseline:** Single-camera DJI Mini 3 Pro mapping experiments are documented in the [experiments-house-model](https://github.com/symmatree/fables/blob/main/Datasets/experiments-house-model.md) log. They identify autofocus, nearfield parallax, and turnaround gimbal instability as the dominant problems -- not rolling shutter. The Rekon's architecture is a response to those single-camera limitations.
