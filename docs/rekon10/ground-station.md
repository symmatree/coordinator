...
The backpack's MAVLink UDP target is learned at runtime, not configured here -- the leading (unconfirmed) theory for the recurring no-RTK failure. See coordinator [`docs/rtk-corrections-path.md`](../../docs/rtk-corrections-path.md).
...
See [ardupilot.md](ardupilot.md) for Rekon10 ArduPilot channel and parameter notes. See [Firefly16 docs](https://github.com/symmatree/fables/blob/main/Drones/flywoo-firefly16/firefly16.md) for Betaflight config.
