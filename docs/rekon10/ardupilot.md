...
**Config now lives in the coordinator repo, decomposed by subsystem:** coordinator [`ardupilot/`](../../ardupilot/README.md) holds the ground-truth Mission Planner export (`rekon10-methodi.param`) plus [`inputs/*.param`](../../ardupilot/inputs) -- one commented fragment per subsystem, overrides only, each param carrying its rationale and provenance. **Those fragments are authoritative for the actual values;** the tables and `PARAM = value` mentions in this doc are for context and can lag -- when they disagree, trust the fragment. Section pointers below name the specific fragment.
...
Values: [`ardupilot/inputs/`](../../ardupilot/inputs) -- `SERIAL*` params sit with the subsystem that owns the port (GPS in `gps-compass.param`, VIO in `ekf-vio.param`, ESC telemetry in `esc-motors-notch.param`, VTX in `vtx-osd.param`). Wiring: [flight-platform.md](flight-platform.md). The table below is the wiring map for context; the fragment is authoritative for `PROTOCOL` / `BAUD`.
...
*Wiring resolved 2026-07 from the recorded pigtail pads + the param export ([`ardupilot/`](../../ardupilot/README.md) in coordinator), which agree with the [flight-platform.md](flight-platform.md) table. An earlier plan to move the F9P to SERIAL7 was abandoned -- it stayed on SERIAL2 (the old M100 port), and SERIAL7 is unused.*
...
Corrections delivery path and the leading no-RTK-failure theory: coordinator [`docs/rtk-corrections-path.md`](../../docs/rtk-corrections-path.md).
...
All **`MOT_PWM_*`**, **`SERVO_BLH_*`**, **`SERIAL8_*`**, motor **`SERVO*_FUNCTION`**, and the harmonic-notch set: **[`inputs/esc-motors-notch.param`](../../ardupilot/inputs/esc-motors-notch.param)** (motor endpoints in [`radio-cal.param`](../../ardupilot/inputs/radio-cal.param)).
